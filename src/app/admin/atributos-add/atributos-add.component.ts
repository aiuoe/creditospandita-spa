...
@Component({
  selector: 'app-atributos-add',
  templateUrl: './atributos-add.component.html',
  styleUrls: ['./atributos-add.component.scss']
})
export class AtributosAddComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.activatedRoute.params
      .pipe(
        switchMap(params => {
          if (params['id']) {
            return this.AtributosService.show(params['id']);
          } else {
            return of(null);
          }
        })
      ...
      .subscribe(blog => {
        if (blog) {
          ...
          this.formBlog.controls['ciudads'].setValue(blog[0]['variable']);
          this.formBlog.controls['variable'].setValue(blog[0]['variable']);
          this.formBlog.controls['categoria'].setValue(blog[0]['categoria']);
  ...
  add() {
    if (this.formBlog.valid) {
      ...
      this.AtributosService.add(this.formBlog.value).subscribe(response => {
        if (response) {
