...
export class SidebarDirective implements OnInit, AfterViewInit {
  ...
  onOutsideClick(event) {
    ...
    if (this.innerWidth < 992) {
      ...
      do {
        ...
        if(clickedComponent.hasAttribute('ng-reflect-classes') && clickedComponent.getAttribute('ng-reflect-classes') === 'has-sub' ){
          submenus=true;
        ...
        clickedComponent = clickedComponent.parentElement;
     
      } while (clickedComponent);
      // console.log("1",this.el.nativeElement.classList.contains("hide-sidebar"));
      // console.log("2",inside);
      // console.log("3",event.target.classList.contains("navbar-toggle"));
      if (
        ...
        if(event.target.localName=='span'){
          ...
          console.log("li",li);
          if((li.getAttribute('ng-reflect-level')==1 && submenus==false) || (li.getAttribute('ng-reflect-level')==2 && submenus==true)  ){
            this.toggleHideSidebar.emit(true);
          }
        ...
        }else if((event.target.localName=='a' && submenus==false)  ){
          console.log("etiqueta a");
          this.toggleHideSidebar.emit(true);
        }else if(event.target.localName=='a' && submenus==true){
          let li = event.target.parentElement
          if((li.getAttribute('ng-reflect-level')==2 ) ){
            this.toggleHideSidebar.emit(true);
          }else{
            this.toggleHideSidebar.emit(false);
          }
        }
        // console.log('rabo',event.target.classList)
      }
    }
  }
  ...
  unloadHandler(event: Event) {
    ...
    console.log('borro localstorage')
    return;
  }
  // beforeunloadHandler(event) {
  //     // localStorage.clear();

  //     //  console.log('eventox',event)
